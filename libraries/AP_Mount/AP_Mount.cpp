...
void AP_Mount::init() {
  ...
  for (uint8_t instance = 0; instance < AP_MOUNT_MAX_INSTANCES; instance++) {
    ...
    case Type::Servo:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_Servo(*this, _params[instance], true, instance);
      _num_instances++;
      break;
#endif
    ...
    case Type::SoloGimbal:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_SoloGimbal(*this, _params[instance], instance);
      _num_instances++;
      break;
    ...
    case Type::Alexmos:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_Alexmos(*this, _params[instance], instance);
      _num_instances++;
      break;
#endif
    ...
    case Type::SToRM32:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_SToRM32(*this, _params[instance], instance);
      _num_instances++;
      break;
#endif
    ...
    case Type::SToRM32_serial:
      _backends[instance] = NEW_NOTHROW AP_Mount_SToRM32_serial(
          *this, _params[instance], instance, serial_instance);
      _num_instances++;
      serial_instance++;
      break;
#endif
    ...
    case Type::MAVLink:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_MAVLink(*this, _params[instance], instance);
      _num_instances++;
      break;
    ...
    case Type::BrushlessPWM:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_Servo(*this, _params[instance], false, instance);
      _num_instances++;
      break;
#endif
    ...
    case Type::Siyi:
      _backends[instance] = NEW_NOTHROW AP_Mount_Siyi(
          *this, _params[instance], instance, serial_instance);
      _num_instances++;
      serial_instance++;
      break;
    ...
    case Type::Scripting:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_Scripting(*this, _params[instance], instance);
      _num_instances++;
      break;
    ...
    case Type::Xacti:
      _backends[instance] =
          NEW_NOTHROW AP_Mount_Xacti(*this, _params[instance], instance);
      _num_instances++;
      break;
    ...
    // check for Xacti gimbal
    case Type::Viewpro:
      _backends[instance] = NEW_NOTHROW AP_Mount_Viewpro(
          *this, _params[instance], instance, serial_instance);
      _num_instances++;
      serial_instance++;
      break;
    ...
    case Type::Topotek:
      _backends[instance] = NEW_NOTHROW AP_Mount_Topotek(
          *this, _params[instance], instance, serial_instance);
      _num_instances++;
      serial_instance++;
      break;
    ...
    case Type::CADDX:
      _backends[instance] = NEW_NOTHROW AP_Mount_CADDX(
          *this, _params[instance], instance, serial_instance);
      _num_instances++;
      serial_instance++;
      break;
    ...
    case Type::XFRobot:
      _backends[instance] = NEW_NOTHROW AP_Mount_XFRobot(
          *this, _params[instance], instance, serial_instance);
      _num_instances++;
      serial_instance++;
      break;
  ...
  // init each instance, do it after all instances were created, so that they
  // all know things
  for (uint8_t instance = 0; instance < AP_MOUNT_MAX_INSTANCES; instance++) {
    if (_backends[instance] != nullptr) {
...
// has_pan_control - returns true if the mount has yaw control (required for
// copters)
...
// get_mode - returns current mode of mount (i.e. Retracted, Neutral,
// RC_Targeting, GPS Point)
...
// set_mode_to_default - restores the mode to it's default mode held in the
// MNTx__DEFLT_MODE parameter
...
// set yaw_lock used in RC_TARGETING mode.  If true, the gimbal's yaw target is
// maintained in earth-frame meaning it will lock onto an earth-frame heading
// (e.g. North) If false (aka "follow") the gimbal's yaw is maintained in
// body-frame meaning it will rotate with the vehicle
...
// set roll_lock used in RC_TARGETING mode.  If true, the gimbal's roll target
// is maintained in earth-frame If false (aka "follow") the gimbal's roll is
// maintained in body-frame meaning it will rotate with the vehicle
...
// set pitch_lock used in RC_TARGETING mode.  If true, the gimbal's tilt target
// is maintained in earth-frame If false (aka "follow") the gimbal's tilt is
// maintained in body-frame meaning it will tilt with the vehicle
...
// yaw_is_earth_frame (aka yaw_lock) should be true if yaw angle is earth-frame,
// false if body-frame
void AP_Mount::set_angle_target(uint8_t instance, float roll_deg,
...
// yaw_lock should be true if the yaw rate is earth-frame, false if body-frame
// (e.g. rotates with body of vehicle)
void AP_Mount::set_rate_target(uint8_t instance, float roll_degs,
                               float pitch_degs, float yaw_degs,
                               bool yaw_lock) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return;
...
MAV_RESULT AP_Mount::handle_command_do_mount_configure(
...
MAV_RESULT
AP_Mount::handle_command_do_mount_control(const mavlink_command_int_t &packet) {
  auto *backend = get_primary();
  if (backend == nullptr) {
    return MAV_RESULT_FAILED;
  }
...
MAV_RESULT AP_Mount::handle_command_do_gimbal_manager_pitchyaw(
    const mavlink_command_int_t &packet) {
  ...
  for (uint8_t i = 0; i < AP_MOUNT_MAX_INSTANCES; i++) {
    if (instance != 0 && instance != (i + 1)) {
      continue;
    }
    auto *backend = get_instance(i);
    if (backend == nullptr) {
      continue;
    }
    ...
    if (!isnan(pitch_angle_deg) && !isnan(yaw_angle_deg)) {
      backend->set_angle_target(0, pitch_angle_deg, yaw_angle_deg,
    ...
    if (!isnan(pitch_rate_degs) && !isnan(yaw_rate_degs)) {
      backend->set_rate_target(0, pitch_rate_degs, yaw_rate_degs,
                               flags & GIMBAL_MANAGER_FLAGS_YAW_LOCK);
      result = MAV_RESULT_ACCEPTED;
      continue;
    }

    // if neither angles nor rates were provided set the RC_TARGETING yaw lock
    // state
    if (isnan(pitch_angle_deg) && isnan(yaw_angle_deg) &&
        isnan(pitch_rate_degs) && isnan(yaw_rate_degs)) {
...
// handle mav_cmd_do_gimbal_manager_configure for deconflicting different
// mavlink message senders
MAV_RESULT AP_Mount::handle_command_do_gimbal_manager_configure(
    const mavlink_command_int_t &packet, const mavlink_message_t &msg) {
  MAV_RESULT result = MAV_RESULT_FAILED;

  // check gimbal device id.  0 is primary, 1 is 1st gimbal, 2 is 2nd gimbal,
  // etc
  const uint8_t instance = packet.z;
  ...
  for (uint8_t i = 0; i < AP_MOUNT_MAX_INSTANCES; i++) {
    ...
    if (backend->handle_command_do_gimbal_manager_configure(packet, msg) ==
        MAV_RESULT_ACCEPTED) {
      result = MAV_RESULT_ACCEPTED;
    }
  }
...
// handle GIMBAL_MANAGER_SET_ATTITUDE message
void AP_Mount::handle_gimbal_manager_set_attitude(
  ...
  for (uint8_t i = 0; i < AP_MOUNT_MAX_INSTANCES; i++) {
    ...
    const Vector3f att_rate_degs{packet.angular_velocity_x,
    ...
    if (!att_quat.is_nan()) {
      ...
      backend->set_angle_target(attitude.x, attitude.y, attitude.z,
                                flags & GIMBAL_MANAGER_FLAGS_YAW_LOCK);
      continue;
    }

    {
      ...
      backend->set_rate_target(roll_rate_degs, pitch_rate_degs, yaw_rate_degs,
                               flags & GIMBAL_MANAGER_FLAGS_YAW_LOCK);
      continue;
    }
  }
...
// handle GIMBAL_MANAGER_SET_PITCHYAW message
void AP_Mount::handle_gimbal_manager_set_pitchyaw(
    const mavlink_message_t &msg) {
  ...
  for (uint8_t i = 0; i < AP_MOUNT_MAX_INSTANCES; i++) {
    ...
    if (!isnan(packet.pitch) && !isnan(packet.yaw) &&
        !isnan(packet.pitch_rate) && !isnan(packet.yaw_rate)) {
      continue;
    }
    ...
    // if neither angles nor rates were provided set the RC_TARGETING yaw lock
    // state
    if (isnan(packet.pitch) && isnan(packet.yaw) && isnan(packet.pitch_rate) &&
        isnan(packet.yaw_rate)) {
      backend->set_yaw_lock(flags & GIMBAL_MANAGER_FLAGS_YAW_LOCK);
      continue;
    }
  }
...
MAV_RESULT
AP_Mount::handle_command_do_set_roi_sysid(const mavlink_command_int_t &packet) {
  set_target_sysid((uint8_t)packet.param1);
  return MAV_RESULT_ACCEPTED;
}

MAV_RESULT AP_Mount::handle_command(const mavlink_command_int_t &packet,
                                    const mavlink_message_t &msg) {
...
// get poi information.  Returns true on success and fills in gimbal attitude,
// location and poi location
bool AP_Mount::get_poi(uint8_t instance, Quaternion &quat, Location &loc,
                       Location &poi_loc) const {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return false;
  }
...
// get mount's current attitude in euler angles in degrees.  yaw angle is in
// body-frame returns true on success
bool AP_Mount::get_attitude_euler(uint8_t instance, float &roll_deg,
                                  float &pitch_deg, float &yaw_bf_deg) {
...
bool AP_Mount::get_rate_target(uint8_t instance, float &roll_degs,
                               float &pitch_degs, float &yaw_degs,
                               bool &yaw_is_earth_frame) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
  ...
  return backend->get_rate_target(roll_degs, pitch_degs, yaw_degs,
                                  yaw_is_earth_frame);
}

// get target angle in deg. returns true on success
bool AP_Mount::get_angle_target(uint8_t instance, float &roll_deg,
                                float &pitch_deg, float &yaw_deg,
                                bool &yaw_is_earth_frame) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
  ...
  return backend->get_angle_target(roll_deg, pitch_deg, yaw_deg,
                                   yaw_is_earth_frame);
}
...
void AP_Mount::set_attitude_euler(uint8_t instance, float roll_deg,
                                  float pitch_deg, float yaw_bf_deg) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return;
...
// set_roi_target - sets target location that mount should attempt to point
// towards
...
// clear_roi_target - clears target location that mount should attempt to point
// towards
...
bool AP_Mount::set_zoom(uint8_t instance, ZoomType zoom_type,
                        float zoom_value) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return false;
...
SetFocusResult AP_Mount::set_focus(uint8_t instance, FocusType focus_type,
                                   float focus_value) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
...
// if POINT only p1 is used, if RECTANGLE then p1 is top-left, p2 is
// bottom-right p1,p2 are in range 0 to 1.  0 is left or top, 1 is right or
// bottom
bool AP_Mount::set_tracking(uint8_t instance, TrackingType tracking_type,
...
// set_camera_source is functionally the same as set_lens except primary and
// secondary lenses are specified by type primary and secondary sources use the
// AP_Camera::CameraSource enum cast to uint8_t
bool AP_Mount::set_camera_source(uint8_t instance, uint8_t primary_source,
...
void AP_Mount::send_camera_information(uint8_t instance,
...
void AP_Mount::send_camera_settings(uint8_t instance,
                                    mavlink_channel_t chan) const {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return;
...
void AP_Mount::send_camera_capture_status(uint8_t instance,
                                          mavlink_channel_t chan) const {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return;
...
void AP_Mount::send_camera_thermal_range(uint8_t instance,
                                         mavlink_channel_t chan) const {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return;
...
bool AP_Mount::change_setting(uint8_t instance, CameraSetting setting,
                              float value) {
  auto *backend = get_instance(instance);
  if (backend == nullptr) {
    return false;
...
bool AP_Mount::get_rangefinder_distance(uint8_t instance,
...
void AP_Mount::handle_message(mavlink_channel_t chan,
                              const mavlink_message_t &msg) {
  switch (msg.msgid) {
...
void AP_Mount::handle_gimbal_device_attitude_status(
    const mavlink_message_t &msg) {
  for (uint8_t instance = 0; instance < AP_MOUNT_MAX_INSTANCES; instance++) {
    if (_backends[instance] != nullptr) {
...
void AP_Mount::convert_params() {
  ...
  int8_t mnt_type = 0;
  IGNORE_RETURN(
      AP_Param::get_param_by_index(this, 19, AP_PARAM_INT8, &mnt_type));
  if (mnt_type == 0) {
    ...
    int8_t stab_pitch = 0;
    IGNORE_RETURN(
        AP_Param::get_param_by_index(this, 4, AP_PARAM_INT8, &stab_roll));
    IGNORE_RETURN(
        AP_Param::get_param_by_index(this, 5, AP_PARAM_INT8, &stab_pitch));
    ...
    // if the mount was previously set, then we need to save the upgraded mount
    // type
  ...
  int8_t jstick_spd = 0;
  if (AP_Param::get_param_by_index(this, 16, AP_PARAM_INT8, &jstick_spd) &&
      (jstick_spd > 0)) {
  ...
  // struct and array holding mapping between old param table index and new
  // RCx_OPTION value
  ...
  for (uint8_t i = 0; i < ARRAY_SIZE(mnt_rc_conversion_table); i++) {
    int8_t mnt_rcin = 0;
    if (AP_Param::get_param_by_index(this,
                                     mnt_rc_conversion_table[i].old_rcin_idx,
                                     AP_PARAM_INT8, &mnt_rcin) &&
        (mnt_rcin > 0)) {
...
AP_Mount *mount() { return AP_Mount::get_singleton(); }
